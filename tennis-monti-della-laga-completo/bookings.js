...
const DB_KEY = "tennis_monti_laga_db_v1";
...
async function redisCommand(command, ...args) {
  ...
  if (!response.ok) throw new Error(data.error || "Errore Redis");
  return data.result;
}
...
function computeStats(db) {
  ...
  return {
    byName: Object.values(byNameMap).sort((a, b) => a.name.localeCompare(b.name, "it")),
    lastWeekHours,
    averageWeeklyHours
  };
}

function validateBookingRules(db, key, surname, oldSurname) {
  if (!surname || surname === "Torneo Condominiale") return null;

  const { date, hour } = parseKey(key);
  if (!date || !hour) return "Cella non valida.";
