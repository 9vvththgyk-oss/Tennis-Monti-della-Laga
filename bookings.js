...
const CLEAN_START_VERSION = "2026-06-20-clean-start-1";
...
async function initDb() {
  ...
  const cleanStart = await db.query(
    "SELECT value FROM app_settings WHERE key = 'clean_start_version';"
  );

  if (cleanStart.rows[0]?.value !== CLEAN_START_VERSION) {
    await db.query("TRUNCATE TABLE bookings, booking_history, stats_slots RESTART IDENTITY;");
    await db.query(`
      INSERT INTO app_settings (key, value, updated_at)
      VALUES ('clean_start_version', $1, NOW())
      ON CONFLICT (key)
...
function getTodayRomeDateString() {
...
async function saveBooking(key, surname) {
  ...
  if (date < getTodayRomeDateString()) {
    const error = new Error(oldSurname && !surname
      ? "Non si possono cancellare prenotazioni dei giorni passati."
      : "Non si possono aggiungere o modificare prenotazioni dei giorni passati.");
    error.statusCode = 400;
    throw error;
  }
  ...
  const summary = await getPool().query(`
    ...
    SELECT
      ...
      CASE
        ...
        ELSE GREATEST(
          0,
          FLOOR((current_week_start - first_week_start) / 7.0)
        )::int
